    ...
    if (interaction.isButton()) {
      ...
      if (interaction.customId.startsWith('enter_giveaway_')) {
        ...
        try {
          ...
          const entryField = embed.fields.find(f => f.name === '🎫 Entries');
          if (entryField) entryField.value = `${gw.entries.length}`;
          await interaction.message.edit({ embeds: [embed] });
        } catch {}

        await interaction.reply({ content: `🎉 You have entered the giveaway for **${gw.prize}**! Good luck!`, ephemeral: true });
      }
    }
...
http.createServer((req, res) => {
  res.writeHead(200);
  res.end('Coral SMP Bot is running!');
}).listen(PORT, () => {
